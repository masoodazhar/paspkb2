...
<!-- Default box -->
<div class="box">
<form action="@if(isset($singleRow)) {{ route('faqs.update', ['faq'=>$singleRow->id ]) }} @else {{ route('faqs.store') }} @endif" method="post" enctype="multipart/form-data">
@csrf
@if(isset($singleRow))
{{ method_field('PUT') }}
@endif
  ...
  <div class="box-header with-border">
    <h3 class="box-title">  FAQs Form </h3>
        ...
        <table class="table table-light">
          ...
          @forelse($allRows as $parliamentaryyears)
            <tr>
              ...
              <td>
                <a href="{{ route('faqs.edit', ['faq' => $parliamentaryyears->id]) }}" class="btn btn-primary">Edit</a> 
                <form style="width: 47%; float: right;" action="{{ route('faqs.destroy', ['faq'=> $parliamentaryyears->id ]) }}" onSubmit="return confirm('Are Your Sure to Delete?')" class="form-inline" method="post">
                    @csrf
                    {{ method_field('DELETE') }}                    
