...
<div class="box">
  ...
  <form action="@if(isset($singleRow)){{ route('publicaccountscommittee.update', ['publicaccountscommittee'=>$singleRow->id ]) }} @else {{ route('publicaccountscommittee.store') }} @endif" method="post" enctype="multipart/form-data">
          @csrf
          @if(isset($singleRow))
              {{ method_field('PUT') }}
          @endif
  <div class="box-header with-border">
    <h3 class="box-title">  Committees Form </h3>
        ...
        <table class="table table-light">
          ...
          @forelse($allRows as $parliamentaryyears)
            <tr>
              ...
              <td>
                <a href="{{ route('publicaccountscommittee.edit', ['publicaccountscommittee' =>$parliamentaryyears->id]) }}" class="btn btn-primary">Edit</a> 
                <form style="width: 47%; float: right;" action="{{ route('publicaccountscommittee.destroy', ['publicaccountscommittee'=>$parliamentaryyears->id ]) }}" onSubmit="return confirm('Are Your Sure to Delete?')" class="form-inline" method="post">
                    @csrf
                    {{ method_field('DELETE') }}                    
