...
<!-- Default box -->
<div class="box">
<form action="@if(isset($singleRow)) {{ route('publications.update', ['publication'=>$singleRow->id ]) }} @else {{ route('publications.store') }} @endif" method="post" enctype="multipart/form-data">
@csrf
@if(isset($singleRow))
{{ method_field('PUT') }}
@endif
  <div class="box-header with-border">
    <h3 class="box-title">  Publications, Reports Form</h3>
        ...
        <div class="row">
            ...
            <div class="col-md-3">
                <div class="form-group">
                    ...
                    <select name="department_committee" class="form-control">
            ...
            <div class="col-md-3">
                <div class="form-group">
                    ...
                    <select name="page" id="" class="form-control">
                        <option value="Publication" @if(isset($singleRow))@if($singleRow->page == 'Publication') selected @endif @endif>Publication</option>
                        <option value="Reports" @if(isset($singleRow))@if($singleRow->page == 'Reports') selected @endif @endif>Reports</option>
                    </select>
                </div>
            </div>
        ...
        <div class="row">
            <div class="col-md-3">                  
                <div class="form-group">
                    ...
                    <input type="date" class="form-control"  value="@if(isset($singleRow)){{$singleRow->date}}@endif" name="date" >
                </div>
            </div>
            <div class="col-md-3">                  
                <div class="form-group">
                    <label for="">type/department</label>
                    <input type="text" class="form-control" value="@if(isset($singleRow)){{$singleRow->department_type}}@endif" name="department_type" >
                </div>
            </div>
            <div class="col-md-3">                  
                <div class="form-group">
...
<div class="box">
  ...
  <div class="box-header with-border">
    <h3 class="box-title">Publications, Reports List</h3>
  ...
  <div class="box-body">
        <table class="table table-light">
          <thead class="thead-light">
            <tr>
                <th width="10%">Assembly Tenures</th>
                <th width="30%">Title</th>
                <th width="12%">Department Committee</th>
                <th width="10%">Date</th>
                <th width="13%">Department Type</th>
                <th width="10%">Page</th>
          ...
          @forelse($allRows as $parliamentaryyears)
            <tr>
              ...
              <td>{{$parliamentaryyears->department_committee}}</td>
              <td>{{$parliamentaryyears->date}}</td>
              <td>{{$parliamentaryyears->department_type}}</td>
              <td>{{$parliamentaryyears->page}}</td>
              <td>
                <a href="{{ route('publications.edit', ['publication' => $parliamentaryyears->id]) }}" class="btn btn-primary">Edit</a> 
                <form style="width: 47%; float: right;" action="{{ route('publications.destroy', ['publication'=> $parliamentaryyears->id ]) }}" onSubmit="return confirm('Are Your Sure to Delete?')" class="form-inline" method="post">
                    @csrf
                    {{ method_field('DELETE') }}                    
