...
<!-- Default box -->
<div class="box">
<form action="@if(isset($singleRow)) {{ route('rules.update', ['rule'=>$singleRow->id ]) }} @else {{ route('rules.store') }} @endif" method="post" enctype="multipart/form-data">
                @csrf
                @if(isset($singleRow))
                {{ method_field('PUT') }}
                @endif
  <div class="box-header with-border">
    <h3 class="box-title">  Rules Form</h3>
        ...
        <table class="table table-light">
          ...
          @forelse($allRows as $about)
            <tr>
              ...
              <td>{{$about->type}}</td>
              <td>
                <a href="{{ route('rules.edit', ['rule' => $about->id]) }}" class="btn btn-primary">Edit</a> 
                <form style="width: 47%; float: right;" action="{{ route('rules.destroy', ['rule'=> $about->id ]) }}" onSubmit="return confirm('Are Your Sure to Delete?')" class="form-inline" method="post">
                        @csrf
                        {{ method_field('DELETE') }}
