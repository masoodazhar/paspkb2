...
<div class="box">
  ...
  <div class="box-header with-border">
    <h3 class="box-title">Speakers Form</h3>
  ...
  <div class="box-body">
        <form action="{{ route('speakers.update', ['speaker'=>$singleRowMain->id ])}} " method="post">
            @csrf
            {{ method_field('PUT') }}
            ...
            <div class="row">
                    ...
                    <div class="form-group">
                        ...
                        <select name="members_directories_id" id=""  class="form-control">
                            @forelse($memberDirectories as $assembly)
                                <option value="{{$assembly->id}}" @if($singleRowMain)@if($singleRowMain->members_directories_id == $assembly->id) selected @endif @endif  >{{$assembly->name}}</option>
                            @empty
                ...
                <div class="col-md-12">                  
                    <div class="form-group">
                        ...
                        <textarea name="speakermessage" class="form-control summernote" id="" cols="20" rows="10">@if(isset($singleRowMain)) {{$singleRowMain->speakermessage}} @endif</textarea>
                    </div>
                </div>
                <div class="col-md-12">                  
                    <div class="form-group">
                        <label for="">Speaker's Role in the Assembly</label>
                        <textarea name="speakersrole" class="form-control summernote" id="" cols="20" rows="10">@if(isset($singleRowMain)) {{$singleRowMain->speakersrole}} @endif </textarea>
                    </div>
                </div>
            </div>
            <div class="row">
        ...
        <hr>
        <form action="@if(isset($singleRow)) {{ route('speakers.update', ['speaker'=>$singleRow->id ]) }} @else {{ route('speakers.store') }} @endif" method="post" enctype="multipart/form-data">
                @csrf
                @if(isset($singleRow))
                {{ method_field('PUT') }}
                @endif
...
<div class="box">
  ...
  <div class="box-header with-border">
    <h3 class="box-title">Speakers Data</h3>
